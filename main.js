...
function updateCanvas(){
    background("white");
    random_number = Math.floor((Math.random() * Quick_draw_array.length)+1);
    random_sketch = Quick_draw_array[random_number];
    console.log(random_sketch);
    document.getElementById("Red_color").innerHTML = "You have to draw a " + random_sketch; 
}

function checkSketch(){
        timer_counter = timer_counter + 1;
        document.getElementById("timer1").innerHTML = "Timer : " + timer_counter;
        console.log(timer_counter);
        if(timer_counter >= 750){
            timer_counter = 0;
